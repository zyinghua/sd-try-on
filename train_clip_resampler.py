...
class Resampler(nn.Module):
    """IP-Adapter-Plus Resampler: maps CLIP patch tokens -> num_queries IP tokens at cross_attention_dim.
...
def log_validation(
    ...
    for idx, (validation_prompt, validation_image) in enumerate(zip(validation_prompts, validation_images)):
        ...
        # Encode cloth image with CLIP -> patch tokens (penultimate hidden state) for Resampler
        ...
        with torch.no_grad():
            cloth_image_embeds = image_encoder(
                clip_pixel_values, output_hidden_states=True
            ).hidden_states[-2]
...
def main(args):
    ...
    # Initialize IP-Adapter projection model (IP-Adapter-Plus Resampler).
    # Consumes CLIP penultimate-layer patch tokens (dim = hidden_size) and produces
    # `num_queries` IP tokens at cross_attention_dim for decoupled cross-attn.
    image_proj_model_ip_adapter = Resampler(
        dim=unet.config.cross_attention_dim,
        depth=4,
        dim_head=64,
        heads=16,
        num_queries=args.num_ip_tokens,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                # ======================= IP-Adapter =========================
                # Use CLIP penultimate-layer patch tokens (N_patches+1 tokens) for the Resampler,
